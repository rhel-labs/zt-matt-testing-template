...
There is no `ps` output because the process is no longer running. 
The `Terminated` output is from the shell's job control system 
which was managing the `dd` process in the background. 
